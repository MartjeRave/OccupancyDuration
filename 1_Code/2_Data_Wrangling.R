...
#### Cleaning of Data  ####
## making the data set complete even if no data has been recorded that day.

complete_data<-as.data.frame(rep(as.Date(min(Divi$daten_stand):max(Divi$daten_stand),  
                                         origin = "1970-01-01"),
...
##### This is not used in this paper, however if you are looking to differentiate between 
##### sex then you may use the following code (l. 130- 138)
...
#####################
#### Lagged Variables (infection rates of the previous week)
#####################
...
districts<-preprocess.districts(path.LRZ = functionpath)
...
load(paste0(functionpath, "/2. Data/district_data.RData"))
...
depri<-read_excel(paste0(functionpath, "/2. Data/deprivation.xlsx"))
