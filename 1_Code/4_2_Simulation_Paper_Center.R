...
######################################################
######### Simulation Model Application- Center Cov ###
######################################################
...
for(i in varianceRuns){
  ...
  VCOV_Coef_Runs_s[(i)-min(varianceRuns)+1,,]<-
    vcov(modellist[[i]])
  check_var_b<-rbind(check_var_b, diag(VCOV_Coef_Runs_b[(i)-min(varianceRuns)+1,,]))
  check_var_s<-rbind(check_var_s, diag(VCOV_Coef_Runs_s[(i)-min(varianceRuns)+1,,]))
  
}


ggplot()+
  geom_line(aes(x=1:nrow(check_var_s), y=check_var_s[,1], col="beta_0"))+
  geom_line(aes(x=1:nrow(check_var_s), y=check_var_s[,2], col="beta_1"))+
  geom_line(aes(x=1:nrow(check_var_s), y=check_var_s[,3], col="beta_2"))+
  xlab("Iterations after convergence")+
  ylab("Variance estimation")+
  ggtitle("Variance estimation at each iteration")

ggplot()+
  geom_line(aes(x=1:nrow(check_var_b), y=check_var_b[,1], col="beta_0"))+
  geom_line(aes(x=1:nrow(check_var_b), y=check_var_b[,2], col="beta_1"))+
  geom_line(aes(x=1:nrow(check_var_b), y=check_var_b[,3], col="beta_2"))+
  xlab("Iterations after convergence")+
  ylab("Square beta-median")+
  ggtitle("Square beta-median at each iteration")
...
Exit_rates<-ggplot()+
  ...
  geom_smooth(aes(x=1:(max_lag-1), y=(apply(omegas_adj[ varianceRuns,], 
                                        2, median))[-12], col="Smooth over Est. Exit"),
              alpha=0.3)+
  ...
  geom_point(aes(x=1:11, y=c(w_true_Grid[1,], 0), col="Ground Truth"), size=2)+
...
ggsave(Exit_rates, file="4. Simulation/exitrateBias_centgamma_centgamma.pdf", width =5, height=3 )
...
ggsave(plot_grid(Exit_rates, chat_plot), file="EstimatedBias_centgamma.pdf",
       width=13, height=4)
...
ggsave(Est_True, file="4. Simulation/InOutEstSim_centgamma.pdf",
       width=8, height=4)
...
ggsave(Exit_rates_gt, file="4. Simulation/ExitRateGroundTruth_centgamma.pdf",
       width=5, height=3)
...
ggsave(LogLike, file="4. Simulation/LogLikeSim_centgamma.pdf", height=3, width=5)
