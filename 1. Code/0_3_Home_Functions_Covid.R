...
######################################################
### Same as simulation homemade function 
...
Score_func_w<-function(w_score_fun=c(w_true,0), 
  ...
  for(s in 1:length(score_vec)){
    ...
    score_vec[s]<-sum((R_td_score*(I_ts_score-I_tN_score)/(Sum_score+0.00001))-
                        (I_ts_score-I_tN_score)) # Slightly different to simulation
...
rdiffpois_array_paper =function(data_rdiff=preptrain_try, # data
                                ...
                                max.k=1000, # probability calculations cut at maximum 300 incoming units
                                rep_run=TRUE){ # If true set up for lag (impute for dates before first observed date)
  ...
  for(datum in unique(data_rdiff$date)){
    ...
    Out_poss<-IN_poss- matrix(rep(as.matrix(Diff_sim[, 
                                                     names(Diff_sim)==as.character(as.Date(datum))]), 
                                  length(0:max.k)),
    ...
    lambda_out_array[lambda_out_array[, names(lambda_out_array)==
                                        as.character(as.Date(datum))]==0,
    ...
    In_sim[, names(In_sim)==as.character(as.Date(datum))]<-
      apply(Prob, 1, sample, x=0:max.k, size=1, replace=TRUE)
    
    Out_sim[, names(Out_sim)==as.character(as.Date(datum))]<-
      In_sim[, names(In_sim)==as.character(as.Date(datum))]-
