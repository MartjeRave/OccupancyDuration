...
setwd(substr(dirname(rstudioapi::getActiveDocumentContext()$path),1, nchar(dirname(rstudioapi::getActiveDocumentContext()$path))))
...
# ############# Coefficients Median  #############################################
# 
# VCOV_coef<-lapply(modellist[VarianceRuns], vcov)
...
#       round(apply(matrix(unlist(lapply(modellist[VarianceRuns], coef)), 
...
ggsave(plot_grid(mapsmooth, Temp_smooth, rel_heights = c(0.9, 0.1)), file="3. Model/SmoothEstimate40.pdf", width=14, height=5)
...
ggsave(Exit_rates, file="3. Model/ExitRate40.pdf", width=9, height=4)
...
ggsave(est_vs_true_plot, file="3. Model/RKIvsEst40.pdf", height=10, width=10)
...
ggsave(LogLike, file="3. Model/LogLikeCov40.pdf", height=3, width=5)




write.csv(Coeff_in, file="3. Model/Coef_In_Lag_40.csv")
save(modellist, file="3. Model/modellist_Lag_40.R")
save(fitlist, file="3. Model/fitlist_Lag_40.R")
save(VCOV, file="3. Model/VCOV_Coef_Runs_w_s_Lag_40.R")
write.csv(omegas_adj, file="3. Model/Exitrate_In_Lag_40.csv")

# 
# omegas_adj<-read.csv(file="3. Model/Exitrate_In_Lag_40.csv")
# load("3. Model/modellist_Lag_40.R")
# load("3. Model/VCOV_Coef_Runs_w_b_Lag_40.R")
# load("3. Model/VCOV_Coef_Runs_w_b_Lag_40.R")
# 
# load("3. Model/VCOV_Lag_40.R")
