...
optimize_Like<-function(w_opt_prev=w_start, 
                        data_like_opp=Outer1$data_all,
                        failsave=exp(-0.1*1:(max_lag-1))/sum(exp(-0.1*1:(max_lag-1)))){
  ...
  Opt<-tryCatch({solve.QP(Dmat=Dmat,
                dvec=d,
                Amat=Amat,
                bvec=b)}, error=function(e, failsave=failsave){
                  cat("!!!!Had to impute w_start with failsave!!!")
                  return(failsave)
                })
  if(is.list(Opt)){
    return(list("w_sol"=Opt$solution, "Infor"=Dmat, "w_sol_uncon"=Opt$unconstrained.solution))
  }else{
    return(list("w_sol"=Opt, "Infor"=Dmat, "w_sol_uncon"=NA))      
  }
...
optimize_Like_wcheck<-function(w_opt_prev=w_start_adj, 
  ...
  VarFish<-Neg2_der_w(w_derivative=w_opt_prev,
                      data_derivative=Off_data_temp)
...
train_data_l<-readRDS("2. Data/training_data.rds")%>%
  ...
  mutate(Incoming=rpois(n=length(exp(0.3*G_5_7_lag_1)), lambda=exp(0.3*G_5_7_lag_1)))
...
w_start<-exp(-0.4*1:max_lag)/sum(exp(-0.4*1:max_lag))

data_out<-Off_cal(rbind(rep(train_data_l[train_data_l$date==min(train_data_l$date),], length(w_start))%>%
                mutate(date=date-rep(1:length(w_start), each=length(unique(train_data_l$districtId)))),
              train_data_l), w_start)

train_data_l<-merge(train_data_l, cbind(data_out[,c("date", "districtId")], 
              "Outgoing"=rpois(nrow(train_data_l), t(w_start%*%t(data_out[,grepl("Incoming_", names(data_out))])))))
...
w_start<-exp(-0.4*1:max_lag)/sum(exp(-0.4*1:max_lag))
### Save Results
omegas_org<-matrix(0, ncol=max_lag, nrow=runs1)
omegas_jk<-matrix(0, ncol=max_lag, nrow=runs1)
omegas_adj<-matrix(0, ncol=max_lag, nrow=runs1)
modellist<-list()
IncomOutgo<-list()
...
for(z in z:runs1){
  ...
  if(z/runs1<0.5){max_w_start=100}else{max_w_start=3}
  ...
  IncomOutgo[[z]]<-suppressMessages(as.data.frame(Outer7$data[,c("districtId", "date", 
                                                                 "Incoming", "Outgoing")])%>%
                                      full_join(cbind(Sum_z[c("date", "districtId")], 
                                                      "Sum_w"=t(w_start%*%t(Sum_z[, 
...
varRb<-solve(apply(VCOV[250:350,,], c(2,3), mean))+(1+1/(350-250))*cov(omegas_adj[250:350,])[1:(max_lag-1),
                                                                                             1:(max_lag-1)]/(350-250-1)

Exit_rates<-ggplot()+
  geom_hline(aes(yintercept=1/(max_lag), col="1/30"))+
  geom_ribbon(aes(x=1:(max_lag-1), ymin=apply(omegas_adj[250:350,], 
                                              2, median)[-30]/
                    sum(apply(omegas_adj[250:350,], 2, median))-1.96*sqrt(diag(varRb)), 
                  ymax=apply(omegas_adj[250:350,], 2, median)[-30]/
                    sum(apply(omegas_adj[250:350,], 2, median))+
                    1.96*sqrt(diag(varRb)), 
                  fill="95% Confidence interval"))+
  geom_line(aes(x=1:(max_lag-1), y=(apply(omegas_adj[250:350,], 
                                          2, median)/sum(apply(omegas_adj[250:350,], 
                                                               2, median)))[-30],
                col="Exit Rate"))+
  theme_pubr()+
  labs(color=" ")+
  ggtitle("Bias adjusted exit rate")+
  # paste0("Median adjustment scale ", round(median(C_hat[250:350]), 
  #                                          2)))+
...
TotalCases<-readRDS("/Users/martjerave/Documents/GitHub/OccupancyDuration//DIVI_2022-10-27.rds")
...
coeffcients_smooth<-apply(matrix(unlist(lapply(modellist[200:400], coef)), 
                                 byrow=TRUE, ncol=length(coef(modellist[[200]]))), 
                          2,median)[(ncol(predict(modellist[[1]], type="lpmatrix"))-29+1):
                                      ncol(predict(modellist[[1]], type="lpmatrix"))]
BasisFn_smooth<-predict(modellist[[1]], newdata=train_data_l, type="lpmatrix")[,
                                                                               (ncol(predict(modellist[[1]], type="lpmatrix"))-29+1):
                                                                                 ncol(predict(modellist[[1]], type="lpmatrix"))]
...
VCOV_coef<-lapply(modellist[250:350], cov)
VCOV_coef_arr<-array(0 , dim=c(length(1:100), nrow(VCOV_coef[[1]]), ncol(VCOV_coef[[1]])))

for(i in 250:350){
  VCOV_coef_arr[i-249,,]<-as.matrix(VCOV_coef[[i]])
}
...
VCOV_Coef_Runs<-array(0, dim=c(length(varianceRuns), 
                               ncol=ncol(vcov(modellist[[i]])), 
                               nrow=nrow(vcov(modellist[[i]]))))


for(i in varianceRuns){
  VCOV_Coef_Runs[(i)-min(varianceRuns)+1,,]<-
...
sqrt(diag(apply(VCOV_Coef_Runs, c(2,3), mean)))[1:10]
colnames(vcov(modellist[[i]]))[1:10]
stddevadj<-sqrt(diag(apply(VCOV_Exit_Runs, c(2,3), mean)))

stddev<-sqrt(diag(solve(apply(VCOV[364:400,,], c(2,3), mean))))
cbind(stddevadj, stddev)[1:10,]

sqrt(diag(solve(VCOV[400,,])))
...
ggsave(plot_grid(mapsmooth, Temp_smooth, rel_heights = c(0.9, 0.1)), file="/Users/martjerave/Documents/GitHub/OccupancyDuration/SmoothEstimate.pdf", width=14, height=5)
...
cbind(names(coef(modellist[[1]])), round(coef_med, 3), round(stddev_coef, 3))[1:10,]
...
ggsave(Exit_rates, file="/Users/martjerave/Documents/GitHub/OccupancyDuration/ExitRate.pdf", width=7, height=4)
...
ggsave(plot_grid(LogLike, LogLike150, nrow=1), file="/Users/martjerave/Documents/GitHub/OccupancyDuration/LogLikeCov.pdf", height=3, width=9)
